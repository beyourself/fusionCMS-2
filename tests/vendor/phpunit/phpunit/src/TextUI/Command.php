...
class PHPUnit_TextUI_Command
{
    ...
    protected function handleSelfUpdate($upgrade = false)
    {
        ...
        if (!$upgrade) {
            $remoteFilename = sprintf(
                'http://phar.phpunit.de/phpunit-%s.phar',
                file_get_contents(
                    sprintf(
                        'http://phar.phpunit.de/latest-version-of/phpunit-%s',
                        PHPUnit_Runner_Version::series()
                    )
                )
            );
        ...
        // Workaround for http://bugs.php.net/bug.php?id=65538
    ...
    protected function handleVersionCheck()
    {
        ...
        $latestVersion = file_get_contents('http://phar.phpunit.de/latest-version-of/phpunit');
