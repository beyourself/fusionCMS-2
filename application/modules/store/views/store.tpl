				...
				{if $data && $data.items}
					{foreach from=$data.items item=item key=itemId}
						...
						<div class="store_item" id="item_{$item.id}">
							...
							<img class="item_icon" src="http://wow.zamimg.com/images/wow/icons/medium/{$item.icon}.jpg" align="absmiddle" {if $item.tooltip}data-realm="{$selected_realm}" rel="item={$item.itemid}"{/if}>
