	...
	<section class="realm_items">
		...
		{foreach from=$realm.items item=item}
			...
			<article class="store_item" id="checkout_item_{$item.id}">
				...
				<img class="item_icon" src="http://wow.zamimg.com/images/wow/icons/medium/{$item.icon}.jpg" align="absmiddle" {if $item.tooltip}data-realm="{$item.realm}" rel="item={$item.itemid}"{/if}>
