...
class Facebook extends Plugin
{
	...
	public function getNews()
	{
		...
		foreach($facebook_articles['data'] as $key=>$article)
		{
			...
			// Load the avatar through facebook api and uses SSL return to make sure that we don't break http :P
			$this->news_articles[$key]['avatar'] = 'http://graph.facebook.com/'.$this->CI->config->item('facebook_username').'/picture?type=normal&return_ssl_resources=1';
		}
	...
	private function loadFacebookDetails()
	{
		...
		try 
		{
			...
			$news_articles = file_get_contents('http://graph.facebook.com/'.$this->config['facebook_username'].'/feed/?access_token='.$this->generateAccessToken().'&limit='.$this->CI->config->item('news_limit'));
	...
	private function generateAccessToken()
	{
		try
		{
			...
			$token_line = file_get_contents('http://graph.facebook.com/oauth/access_token?grant_type=client_credentials&client_id='.$this->config['facebook_app_id'].'&client_secret='.$this->config['facebook_app_secret']);
