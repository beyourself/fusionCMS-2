...
class Recruitafriend extends MX_Controller
{
    ...
    {
        $this->template->setTitle("Recruit A Friend");
        ...
        $output = $this->template->loadPage("recruit.tpl", $data);

        $this->template->box("Recruit a Friend", $output, true, "modules/recruitafriend/css/style.css");

        /* @AwkwardDev: UCP-linked code.
        $this->template->view($this->template->loadPage("page.tpl", array(
            "module" => "default", 
            "headline" => "<span style='cursor:pointer;' onClick='window.location=\"".$this->template->page_url."ucp\"'>UCP</span> &rarr; Recruit a Friend",
